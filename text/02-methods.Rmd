...
## Participants
```{r uni_prop, warning=F}
data_df <- read_csv(here("data", "data.csv")) #, locale = locale(encoding = "UTF-16LE"))
uni_prop <- data_df |> 
  mutate(uni = ifelse(!is.na(Q15) & Q15 != "Na", "Y", "N")) |> 
  select(uni, lang_group) |> 
  filter(lang_group %in% c("C", "M")) |> 
  table()
uni_chisq <- chisq.test(uni_prop, correct = FALSE)
```
...
Out of the 80 included participants, 73 either attended, were attending, or were about to attend university, of whom 58 were Chinese-speaking and 15 were Malay-speaking. 
These proportions were not significantly different at $\alpha = .05$ ($\chi^2 = $ `r uni_chisq$statistic |> round(2)`, $p = $ `r uni_chisq$p.value |> apa_p()`). 
...
This block allowed participants to become familiar with the task, and also primed them to interpret sentences in a CSE context.
...
In particular, the 12 copula test sentences included 4 sentences for each complement type (NP, AP, PP); these all involved a 3\textsc{sg} subject to control for the form of the copula (i.e., _is_). 
