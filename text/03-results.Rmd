...
[^03-02]: Reported values indicate estimated marginal means and their standard errors.
...
Post-hoc pairwise comparisons using the Tukey method revealed that there were no significant pairwise contrasts for sentences including the copula (all $p >$ `r cop_con_cqt |> filter(q_type == 1) |> pull(adj.p.value) |> min() |> floor_dec(1) |> apa_p(2)`). 
...
There was no interaction between language and complement type ($p >$ `r cop_sum |> filter(grepl("cop_type[12]:lang_group1$", term)) |> pull(p.value) |> min() |> floor_dec(1) |> apa_p(2)`), or language and sentence version ($p =$ `r cop_sum |> filter(term == "q_type1:lang_group1") |> pull(p.value) |> printp()`), or language and both complement type and sentence version ($p >$ `r cop_sum |> filter(grepl("cop_type[12]:q_type1:lang_group1$", term)) |> pull(p.value) |> min() |> floor_dec(1) |> apa_p(2)`).
...
In contrast with the pattern of results for copular constructions, there were no significant interaction effects between past marking type and sentence version (all $p >$ `r past_sum |> filter(grepl("past_type[1-4]:q_type1$", term)) |> pull(p.value) |> min() |> floor_dec(2) |> apa_p(2)`).
There were also no significant interaction effects between past marking type and both question type and MT (all $p >$ `r past_sum |> filter(grepl("past_type[1-4]:q_type1:", term)) |> pull(p.value) |> min() |> floor_dec(2) |> apa_p(2)`).
...
Post-hoc pairwise comparisons using the Tukey method did not reveal any simple effects of MT (all $p >$ `r past_con_lqt |> pull(p.value) |> min() |> floor_dec(2) |> apa_p(2)`), suggesting that Chinese and Malay speakers did not differ in their ratings for either sentence version, although they differed in the _difference_ in ratings between sentences with and without past tense marking. 
...
Post-hoc pairwise comparisons using the Tukey method revealed that there were no significant pairwise contrasts for sentences with past tense marking (all $p >$ `r past_sem_con |> filter(q_type == 1) |> pull(p.value) |> min() |> floor_dec(1) |> apa_p(2)`). 
For sentences without past tense marking, there were no significant pairwise contrasts for Malay MT speakers (all $p >$ `r past_sem_con |> filter(q_type == 2, lang_group == "M") |> pull(p.value) |> min() |> floor_dec(1) |> apa_p(2)`). 
...
both_plot <- ggplot(both,
  ...
  geom_point() +
  geom_errorbar(aes(ymin = avg_rating_2 - 1.96*se_rating_2, 
                    ymax = avg_rating_2 + 1.96*se_rating_2),
                alpha = .5) +
  geom_errorbar(aes(xmin = avg_rating_1 - 1.96*se_rating_1, 
                    xmax = avg_rating_1 + 1.96*se_rating_1),
                alpha = .5) +
...
The correlation in means between the two types of CSE constructions was moderate, $r =$ `r both_cor |> apa_p(2)`, suggesting that participants sometimes (but not always) endorsed both types of CSE constructions with similar ratings.
Additionally, many of the points lay above the $y = x$ line, suggesting that participants generally rated the CSE past tense constructions higher than the CSE copular constructions; this aligns with findings from @leimgruberSingaporeEnglishStructure2013, who identified lack of verbal inflection as being more common than zero copula.
