...
Sentences with a PP complement (`r cop_emm_cqt |> filter(q_type == 2, cop_type == 3) |> pull(estimate)` $\pm$ `r cop_emm_cqt |> filter(q_type == 2, cop_type == 3) |> pull(std.error)`) had higher ratings than sentences with an NP complement (`r cop_emm_cqt |> filter(q_type == 2, cop_type == 1) |> pull(estimate)` $\pm$ `r cop_emm_cqt |> filter(q_type == 2, cop_type == 1) |> pull(std.error)`, $b =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "2 - 3") |> pull(estimate) |> abs()`, $p =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "2 - 3") |> pull(adj.p.value) |> printp()`) and also had higher ratings than sentences with an AP complement (`r cop_emm_cqt |> filter(q_type == 2, cop_type == 2) |> pull(estimate)` $\pm$ `r cop_emm_cqt |> filter(q_type == 2, cop_type == 2) |> pull(std.error)`, $b =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "3 - 1") |> pull(estimate) |> abs()`, $p =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "3 - 1") |> pull(adj.p.value) |> printp()`). 
Sentences with an NP complement were not significantly different than sentences with an AP complement ($b =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "2 - 1") |> pull(estimate) |> abs()`, $p =$ `r cop_con_cqt |> filter(q_type == 2, contrast == "2 - 1") |> pull(adj.p.value) |> printp()`).
