...
One candidate was the aspect of the event semantics, which was noted by @hoDynamicsContactContinuum1993 to also influence the proportion of past marking.
...
To explore the role of event semantics in past tense marking, we evaluated each sentence in terms of the aspectual features of the event described: dynamic or stative, durative or punctual, and telic or atelic ###CITATIONS HERE: DOWTY, VENDLER, OLSEN.
...
past_sem_hist <- ggplot(semantics_hist) +
  geom_col(aes(x = aspectual_class, y = n), fill = "cornflowerblue") +
...
We also included participant and item as random effects.
The full model specification is as follows: `rating ~ aspectual_class * sent_version * mt + (1 | ppt_id) + (1 | item_id)`.[^03-03]
...
Model results suggested that there was again a main effect of sentence version ($b =$ `r past_sem_sum |> filter(term == "q_type1") |> pull(estimate)`, $p =$ `r past_sem_sum |> filter(term == "q_type1") |> pull(p.value)`), and an interaction effect between sentence version and MT ($b =$ `r past_sem_sum |> filter(term == "q_type1:lang_group1") |> pull(estimate)`, $p =$ `r past_sem_sum |> filter(term == "q_type1:lang_group1") |> pull(p.value)`).
...
However, for Chinese MT speakers, sentences with unmarked past activities (`r past_sem_emm_tidy |> filter(q_type == 2, lang_group == "C", aspectual_class == "activity") |> pull(estimate)` $\pm$ `r past_sem_emm_tidy |> filter(q_type == 2, lang_group == "C", aspectual_class == "activity") |> pull(std.error)`) had higher ratings than sentences with unmarked past states (`r past_sem_emm_tidy |> filter(q_type == 2, lang_group == "C", aspectual_class == "state") |> pull(estimate)` $\pm$ `r past_sem_emm_tidy |> filter(q_type == 2, lang_group == "C", aspectual_class == "state") |> pull(std.error)`, $b =$ `r past_sem_con |> filter(q_type == 2, lang_group == "C", group1 == "activity", group2 == "state") |> pull(estimate) |> abs()`, $p =$ `r past_sem_con |> filter(q_type == 2, lang_group == "C", group1 == "activity", group2 == "state") |> pull(p.value)`). 
...
Additionally, many of the points lay above the $y = x$ line, suggesting that participants generally rated the CSE past tense constructions higher than the CSE copular constructions.
...
prof_emm <- emtrends(prof_lm, ~ q_type, var = "proficiency_std") |> 
  tidy()
# prof_con <- contrast(past_emm_2, method = "pairwise") |> 
#   as_tibble() |>
#   separate(contrast, c("group1", "group2"), " - ") |> 
#   mutate(p.signif = cut(p.value,
#                         breaks = c(-Inf, .0001, .001, .01, .05, .1, 1),
...
There was an interaction effect between sentence version and MT proficiency ($b =$ `r prof_sum |> filter(term == "q_type1:proficiency_std") |> pull(estimate)`, $p =$ `r prof_sum |> filter(term == "q_type1:proficiency_std") |> pull(p.value)`), with the estimated marginal trend of MT proficiency being negative for SSE sentences (slope $=$ `r prof_emm |> filter(q_type == 1) |> pull(proficiency_std.trend`) and positive for CSE sentences (slope $=$ `r prof_emm |> filter(q_type == 2) |> pull(proficiency_std.trend`), although neither reached significance (all $p >$ `r prof_emm |> pull(p.value) |> min() |> floor_dec(1)`).

There was also an interaction effect among sentence version, MT, and MT proficiency ($b =$ `r prof_sum |> filter(term == "q_type1:lang_group1:proficiency_std") |> pull(estimate)`, $p =$ `r prof_sum |> filter(term == "q_type1:lang_group1:proficiency_std") |> pull(p.value)`). 
For Chinese MT speakers, the estimated marginal trend of MT proficiency was positive for both SSE (slope $=$ `r prof_int_emm |> filter(q_type == 1, lang_group == "C") |> pull(proficiency_std.trend`) and CSE (slope $=$ `r prof_int_emm |> filter(q_type == 2, lang_group == "C") |> pull(proficiency_std.trend`) sentences, whereas for Malay MT speakers, the estimated marginal trend of MT proficiency was negative for SSE sentences (slope $=$ `r prof_int_emm |> filter(q_type == 1, lang_group == "M") |> pull(proficiency_std.trend`) but positive for CSE sentences (slope $=$ `r prof_int_emm |> filter(q_type == 2, lang_group == "M") |> pull(proficiency_std.trend`).
